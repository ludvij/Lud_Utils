...
public:
    ...
    template <typename T>
    static std::vector<T> SlurpTo(const std::filesystem::path, std::ios_base::openmode mode = std::ios_base::in);
...
namespace Lud
{
Slurper::Slurper(const std::filesystem::path filename, const std::ios_base::openmode mode)
    : m_mode(mode)
{
    Open(filename, mode);
}
...
std::vector<std::string> Slurper::Slurp(const std::filesystem::path filename, const std::ios_base::openmode mode)
{
    Slurper file(filename, mode);
    return file.ReadLines();
}

bool Lud::Slurper::Open(const std::filesystem::path filename, const std::ios_base::openmode mode)
{
    if (is_open())
    {
        Close();
...
Slurper& Slurper::Read(char* data, size_t size)
{
    read(data, size);
    return *this;
}

std::streampos Slurper::Where()
{
    return tellg();
}

Slurper& Slurper::Move(const std::streamoff pos)
{
    seekg(pos);
...
Slurper& Slurper::Move(const std::streamoff offset, const std::ios_base::seekdir dir)
{
    seekg(offset, dir);
...
bool Slurper::HasSpace(const size_t size)
{
